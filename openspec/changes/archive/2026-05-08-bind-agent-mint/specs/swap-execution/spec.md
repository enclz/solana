...
**Reason**: Daily and per-tx spend limits are denominated in the bound mint's units; once the swap input is an arbitrary mint, the comparison `spent_today + amount_in > daily_limit` mixes units and is meaningless. The threat the limits guarded against — operator-driven theft — is neutralized by the new custody pin on `to_token_account.owner`, since swap proceeds cannot leave agent custody. The hourly transaction cap (a unit-free rate limit) is retained.

**Migration**: None. Daily and per-tx limits remain enforced on `execute_transfer` (the only outbound mint-pinned path) and on `execute_lending_op` (deposits/withdrawals are still in the bound mint). Orchestrators that relied on `daily_limit` to throttle swap volume should move that policy offchain or rely on `hourly_tx_cap`.

### Requirement: Counter updates after successful swap

**Reason**: Replaced by the new "Rate limiting on swaps" requirement, which specifies that `spent_today` is no longer touched by swaps and `tx_count_this_hour` is the only counter that increments. Stating this explicitly under a positive requirement avoids ambiguity at archive time.

**Migration**: None. Backend dashboards that read `spent_today` to display swap volume must instead aggregate from the orchestrator's own ledger; the onchain field reflects only outbound transfer + lending volume in the bound mint.
