...
- [x] 6.3 CI workflow green on probe PR #22 (run `25288598960`): all four jobs pass — `anchor build`, `cargo audit + cargo deny`, `cargo test + anchor test` (96 tests across unit + swap + lending + transfer + owner suites), `tarpaulin coverage gate` (informational per 3.4). Earlier failures surfaced two CI wiring bugs fixed in-line: the `program-build` artifact upload missed `test_stubs.so` (anchor build only compiles `[programs.*]` entries from `Anchor.toml` — the workspace member needs an explicit `cargo build-sbf --manifest-path test-programs/test_stubs/Cargo.toml`), and the coverage job downloaded the artifact to `target/` instead of `target/deploy/` so the litesvm test harness couldn't find the binary
