...
- **`docs/` is a submodule of `enclz/.github` pinned to `branch = main`** (see `.gitmodules`). To refresh: `git submodule update --remote --merge docs && git add docs && git commit`. The recorded SHA still has to be bumped per commit — `branch = main` only tells `--remote` which ref to fast-forward to.
